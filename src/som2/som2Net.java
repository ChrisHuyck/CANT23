...
public class som2Net extends CANTNet {
  private boolean fiveCAs = false;
  private boolean someInternalSupport = true;
  ...
  public som2Net(String name,int cols, int rows,int topology){
...
public void zeroIncomingStrengths(){
  ...
  public CANTNet getNewNet(String name,int cols, int rows,int topology){
    som2Net net = new som2Net (name,cols,rows,topology);
    return (net);
  } 
	...
	protected void createNeurons() {
		...
		else System.out.println("error in som2Net.createNuerons ");
	}
  
	public void connectBaseToSomNet(som2Net somNet) {
  ...
  private void setInputTopology() {
	...
	private void setSOMTopology() {
		if (fiveCAs) {//preset SOM to have 5 CAs.
  ...
  private void printToCAWeights(CANTNeuron neuron) {
    som2Net baseNet = (som2Net) CANT23.experiment.getNet("BaseNet");
    som2Net outputNet = (som2Net) CANT23.experiment.getNet("OutputNet");
    ...
    private int getGasNeuronsFired () {
      ...
      som2Net gasNet = (som2Net) CANT23.experiment.getNet("GasNet");
...
//System.out.println(fromOffset + " hebb compense + " + connectionStrength + " " + modification); 
  ...
  public void learn() {
    ...
    else if (learningOn == 1)
    {
    	...
    	System.out.println(" bad learning on in som2Net learn " + learningOn); 
 }
  
  private int somOn = -1;

	private void printSOMOn() {
		som2Net somNet = (som2Net) CANT23.experiment.getNet("SomNet");
  ...
  private void printSOMPlace(int CA){
    ...
    som2Net inputNet = (som2Net) CANT23.experiment.getNet("BaseNet");
	double xWeights[];
	int xSynapses[];
	double yWeights[];
	int ySynapses[];
  ...
  public void kludge () {
    System.out.println("kludge" + som2CANT.kludge );
    som2Net net = (som2Net) CANT23.experiment.getNet("SomNet");

    if (som2CANT.kludge == 0) {
      net = (som2Net) CANT23.experiment.getNet("BaseNet");
      for (int i= 0; i < 10; i++) 
        {
    ...
    else if (som2CANT.kludge == 2) {
    	for (int i = 0; i < 10; i++)
    	  printSOMPlace(i);
    }
    else if (som2CANT.kludge == 3) {
        for (int i= 0; i < 10; i++) 
          {
