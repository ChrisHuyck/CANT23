...
public class xor4Experiment extends CANTExperiment {

  public xor4Experiment () {
  ...
  public boolean experimentDone(int CANTStep) {
	if ((correct+inCorrect) >= 100) 
		{
        System.out.println(Xor4CANT.getNumSystems() + " Yes " + correct + " No " + inCorrect);
	...
	public void switchToSpontaneousOnly() {
		xor4Net gasNet = (xor4Net) getNet("GasNet");
		xor4Net outputNet = (xor4Net) getNet("OutputNet");
		xor4Net inputNet = (xor4Net) getNet("BaseNet");

		//System.out.println("swithctoSpont ");
		Xor4CANT.inputModel = 3;
  ...
  public void endEpoch() {
    xor4Net  outputNet = (xor4Net)getNet("OutputNet");
    xor4Net  baseNet = (xor4Net)getNet("BaseNet");

    //Xor4CANT.resetForNewTest();

    if (Xor4CANT.inputModel == 0)
      {
  ...
  public double printConnectionWeights(int step, String netName) {
	double totWeight = 0;
	int posNeurons = 0;
	xor4Net net = (xor4Net) getNet(netName);
...
/*
		if (CANTStep % 20 == 9) {
			int numGasFired = 0;
			for (int i = 0; i < gasNet.getSize(); i++) {
				if (gasNet.neurons[i].getFired())
					numGasFired++;
			}
			System.out.println(CANTStep + " " + numGasFired);
		}
*/
		...
		if (inTest) {
			...
			if ((CANTStep > (trainingLength + 500)) && ((CANTStep % 20) == 0)) {
				Xor4CANT.resetForNewTest();
				if (debugLevel > 0)
					System.out.print("Test " + CANTStep + " " + Ns + " " + Ys);
				...
				if (Ys > Ns) {
					if (answer == 1)
						...
						inCorrect++;
					if (debugLevel > 0)
						System.out.println(" Yes " + correct + " " + inCorrect);
				} else {
					if (answer == 0)
						correct++;
					else
						inCorrect++;
					if (debugLevel > 0)
						System.out.println(" No " + correct + " " + inCorrect);
				}
			}
		} else {
