...
public class xorNet extends CANTNet {
  
  public xorNet(){
  }
  
  public xorNet(String name,int cols, int rows,int topology){
...
//if (net.getName().compareTo("VerbNet") == 0)   System.out.println(net.neurons[0].getFatigue() +   " verb Neuron " + net.neurons[0].getActivation());
  ...
  public CANTNet getNewNet(String name,int cols, int rows,int topology){
    xorNet net = new xorNet (name,cols,rows,topology);
    return (net);
  } 
  ...
  //--------fWorks-------------------------
  public void connectBaseToGasNet(xorNet gasNet) {
  ...
  public void connectGasToBaseNet(xorNet baseNet) {
    for (int fromNeuron = 0; fromNeuron < getSize(); fromNeuron++) {
      for (int synapse = 0; synapse < 10; synapse++) {
  ...
  public void connectGasToOutputNet(xorNet outputNet) {
    for (int fromNeuron = 0; fromNeuron < getSize(); fromNeuron++) {
      for (int synapse = 0; synapse < 10; synapse++) {
  ...
  public void connectOutputToGasNet(xorNet gasNet) {
    for (int fromNeuron = 0; fromNeuron < getSize(); fromNeuron++) {
      for (int synapse = 0; synapse < 10; synapse++) {
        int toNeuron = CANT23.random.nextInt(gasNet.getSize());
        if (!neurons[fromNeuron].isInhibitory()) 
  ...
  //--------!fWorks-------------------------
  public void connectBaseToOutput(xorNet outputNet) {
    for (int fromNeuron = 0; fromNeuron < getSize(); fromNeuron++) {
  ...
  public void connectOutputToBase(xorNet baseNet) {
    for (int fromNeuron = 0; fromNeuron < getSize(); fromNeuron++) {
      for (int synapse = 0; synapse < 5; synapse++) {
        int toNeuron = CANT23.random.nextInt(baseNet.getSize());
        if (neurons[fromNeuron].isInhibitory()) 
...
*/
  ...
  private void printToCAWeights(CANTNeuron neuron) {
    xorNet baseNet = (xorNet) CANT23.experiment.getNet("BaseNet");
    xorNet outputNet = (xorNet) CANT23.experiment.getNet("OutputNet");
  ...
  public void kludge () {
    System.out.println("kludge" + CANT23xor.kludge );
    xorNet net = (xorNet) CANT23.experiment.getNet("GasNet");

    if (CANT23xor.kludge == 1) {
      net = (xorNet) CANT23.experiment.getNet("BaseNet");
      for (int i= 0; i < 10; i++) 
        {
